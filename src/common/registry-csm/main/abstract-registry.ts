...
export abstract class AbstractRegistryService {
  ...
  public async syncUpdatedKeysWithContract(
    ...
    for (const [currentIndex, currOperator] of currentOperators.entries()) {
      ...
      // used keys must be a gap-free prefix (usedCount === maxUsed + 1, which also forces it to start
      // at 0) that reaches the cursor (maxUsed + 1 >= cursor). A shortfall means a key below the
