...
export class Migration20260917083520 extends Migration {
  async up(): Promise<void> {
    // totalWithdrawnKeys comes from the community/CSM contract, so we clear all synced data and let the keys-update rebuild everything
    // on first sync. This gives 0x02 operators real values before the service starts
    // serving (it returns 425 until the first sync completes)
